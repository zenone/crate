        ...
        with patch("dj_mp3_renamer.core.io.MutagenFile", create=True), \
            ...
            for preview in previews:
                ...
                if preview.status == "will_rename":
                    ...
                    assert preview.dst.name == "Artist - Title.mp3"
...
class TestPreviewCollisionDetection:
    ...
    def test_preview_detects_collisions(self, api_instance, tmp_path):
        ...
        with patch("dj_mp3_renamer.core.io.MutagenFile", create=True), \
            ...
            assert dst_names[0] == "Artist - Title.mp3"
            assert dst_names[1] == "Artist - Title (2).mp3"  # Collision handled
...
class TestPreviewTemplates:
    ...
    def test_preview_with_custom_template(self, api_instance, test_files):
        ...
        with patch("dj_mp3_renamer.core.io.MutagenFile", create=True), \
            ...
            # Should follow custom template
            expected_name = "[8A] 128 - Artist - Title.mp3"
            for preview in previews:
                if preview.status == "will_rename":
                    assert preview.dst.name == expected_name
