...
class TestPreviewEndpoint:
    ...
    def test_preview_with_enhance_metadata(self, client, test_files):
        ...
        with patch("crate.api.renamer.read_mp3_metadata") as mock_read, \
             patch("crate.api.renamer.auto_detect_metadata") as mock_ai:
...
class TestIntegrationFlow:
    ...
    def test_preview_select_subset_execute(self, client, test_files, mock_metadata):
        ...
        with patch("crate.api.renamer.read_mp3_metadata") as mock_read:
            ...
            # Should only process the selected subset
            if status_data["status"] == "completed":
                assert status_data["total"] == len(selected_files)
                assert status_data["results"] is not None
                assert status_data["results"]["total"] == len(selected_files)
