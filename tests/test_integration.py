...
class TestFullRenameWorkflow:
    ...
    def test_full_rename_workflow_success(self, test_mp3_files, api_instance):
        ...
        with patch("crate.core.io.MutagenFile", create=True) as mock_mutagen, \
             patch("crate.api.renamer.read_mp3_metadata") as mock_read:
        ...
        # NOTE: glob treats [] as a character class, so match via suffix check instead.
        renamed_files = [p for p in tmp_path.glob("*.mp3") if " - " in p.name and p.name.endswith(" [128].mp3")]
...
class TestCancellationWorkflow:
    ...
    def test_cancellation_via_progress_callback(self, tmp_path, api_instance):
        ...
        # Verify partial completion (cancellation is best-effort with concurrent workers)
        assert processed_count >= cancel_after, f"Expected >= {cancel_after} processed before cancel, got {processed_count}"

        # We should have renamed at least some files before cancellation.
        renamed = [p for p in tmp_path.glob("*.mp3") if p.name.startswith("Test Artist - Test Title")]
