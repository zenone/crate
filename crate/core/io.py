...
def read_mp3_metadata(
    ...
    try:
        ...
        disc = _first_tag(tags, "TPOS", "discnumber", "DISCNUMBER")
        genre = _first_tag(tags, "TCON", "genre", "GENRE")  # Task #106

        # Task #106: Extract duration from audio info
        duration_seconds = 0
        if hasattr(audio, 'info') and hasattr(audio.info, 'length'):
            duration_seconds = int(audio.info.length)

        # Task #101: Filename fallback for missing track numbers
        # If ID3 track tag is missing, try to extract from filename
        if not track:
            filename = os.path.basename(str(path))
            # Match patterns like "03 - Artist - Title.mp3", "03. Title.mp3", "03_Title.mp3"
            match = re.match(r'^(\d{1,3})\s*[-_.]\s*', filename)
            if match:
                track = match.group(1)
                logger.debug(f"Extracted track number from filename: {track} ({filename})")
        ...
        # Task #106: Format duration as mm:ss
        duration_formatted = ""
        ...
        meta: Dict[str, str] = {
            ...
            "genre": squash_spaces(genre or ""),  # Task #106
            "year": extract_year(date or ""),
            "track": extract_track_number(track or ""),
            "disc": extract_disc_number(disc or ""),
            "duration": duration_formatted,  # Task #106
