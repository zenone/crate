...
detect_bpm_essentia: Optional[Callable[[Path, logging.Logger], Tuple[Optional[str], str]]]
detect_key_essentia: Optional[Callable[[Path, logging.Logger], Tuple[Optional[str], str]]]

try:
    from .audio_analysis_essentia import (
        ESSENTIA_AVAILABLE,
        detect_bpm_essentia,
        detect_key_essentia,
    )
...
def auto_detect_metadata(
    ...
    if not needs_bpm and not needs_key:
        logger.debug("BPM and Key already in tags - skipping detection")
