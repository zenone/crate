...
from ..core.config import DEFAULT_CONFIG, clear_config_cache, load_config, save_config
...
class RenamerAPI:
            ...
            if request.progress_callback:
                try:
                    ...
                    if "cancel" in type(cb_err).__name__.lower():
                        self.logger.info("Operation cancelled before processing started")
                        ...
                        try:
                            ...
                            if request.progress_callback:
                                try:
                                    ...
                                    if "cancel" in type(cb_err).__name__.lower():
                                        cancel_event.set()
                                        self.logger.info("Operation cancelled via progress callback")
                                        for f in pending_futures:
                                            f.cancel()
                                        raise
                                    self.logger.warning(f"Progress callback error: {cb_err}")
                        except Exception as e:
                            # Future raised an exception (or a cancellation bubbled up from the
                            # progress callback). Cancellation should stop the whole operation.
                            ...
                            # Non-cancellation errors are logged; the batch continues.
    ...
    def _rename_one(
        ...
        try:
            if cancel_event and cancel_event.is_set():
                return RenameResult(src=src, dst=None, status="skipped", message="cancelled", metadata=None)
            ...
            if dst is None:
                # Treat metadata read failures as errors (they require user attention).
                if reason and reason.lower().startswith("metadata read error"):
                    self.logger.info("ERR   %s  (%s)", src.name, reason)
                    return RenameResult(src=src, dst=None, status="error", message=reason, metadata=meta)
            ...
            if cancel_event and cancel_event.is_set():
                return RenameResult(src=src, dst=None, status="skipped", message="cancelled", metadata=meta)
        ...
        if write_tags:
            try:
                ...
                if needs_write:
                    ...
                    if write_success:
                        self.logger.info("  Saved enhanced metadata to ID3 tags")
            except Exception as tag_err:
                self.logger.error(f"  Failed to write tags: {tag_err}")
        else:
            self.logger.debug("  Skipping tag write (preview mode)")
    ...
    def analyze_file(self, file_path: Path, write_tags: bool = False, cancel_event: Optional[threading.Event] = None) -> Optional[dict]:
        ...
        Notes:
            ...
            - May take several seconds for full analysis (~10s per file)
