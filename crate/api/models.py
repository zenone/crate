...
class RenameRequest:
    ...
    auto_detect: bool = False  # Auto-detect BPM/Key if missing (slower; off by default)
    progress_callback: Optional[Callable[[int, str], None]] = None  # Called after each file: (count, filename)
    file_paths: Optional[tuple[Path, ...]] = None  # Specific files to process (overrides path scanning)
