...
#
# Recommended entrypoint for end users.
#
# Back-compat: start_crate_web.sh is a thin wrapper that calls this script.
...
setup_https() {
    ...
    if [ "$CA_INSTALLED" = false ]; then
        ...
        if mkcert -install; then
            ...
            echo -e "${RED}✗ Failed to install CA automatically.${NC}"
            echo -e "${YELLOW}   Run this once in an interactive terminal:${NC}"
            echo "     mkcert -install"
            echo -e "${YELLOW}   Then re-run:${NC} ./crate-web.sh"
            return 1
        fi
    else
...
if [ "$HTTPS_ENABLED" = true ]; then
    ...
    UVICORN_CMD="python -m uvicorn web.main:app --host 127.0.0.1 --port $PORT $RELOAD_FLAG"
fi
...
if [ "$HTTPS_ENABLED" = true ]; then
    ...
    echo -e "${YELLOW}   If your browser was already open, you may need to fully restart it once after mkcert installs the local CA.${NC}"
else
...
# Suppress urllib3 LibreSSL warning noise on macOS.
PYTHONWARNINGS="ignore:.*LibreSSL.*" $UVICORN_CMD > "$LOGFILE" 2>&1 &
SERVER_PID=$!
