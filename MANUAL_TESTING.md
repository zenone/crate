...
# Run CLI
crate /tmp/dj_test_cli --dry-run -v
...
```bash
pytest tests/ --cov=crate --cov-report=term
```
...
```bash
crate ~/Music/TEST_RENAME --dry-run -vv
```
**Expected:**
...
# Test recursive
crate ~/Music/TEST_RENAME --recursive --dry-run -v
```

### Test 4e: Custom Template
```bash
crate ~/Music/TEST_RENAME \
...
```bash
crate /path/that/does/not/exist --dry-run
```
...
crate /tmp/test_single.mp3 --dry-run -v
...
python3 -c "import crate"
python3 -c "from crate.core import sanitization"
python3 -c "from crate.core import key_conversion"
python3 -c "from crate.core import metadata_parsing"
python3 -c "from crate.core import template"
python3 -c "from crate.core import io"
python3 -c "from crate.api import models, renamer"
python3 -c "from crate.cli import main"
...
mypy crate/ --config-file pyproject.toml 2>&1 | grep -E "error|Success" || echo "mypy not installed"
```
...
```bash
crate --help | grep -q "template" && echo "✓ Help includes template"
crate --help | grep -q "workers" && echo "✓ Help includes workers"
crate --help | grep -q "recursive" && echo "✓ Help includes recursive"
```
...
echo "1. API Import:"
python3 -c "from crate.api import RenamerAPI; print('✓')"

echo "2. CLI Old Script:"
crate --help > /dev/null 2>&1 && echo "✓"

echo "3. CLI Module:"
python3 -m crate --help > /dev/null 2>&1 && echo "✓"

echo "4. CLI Installed:"
crate --help > /dev/null 2>&1 && echo "✓" || echo "⚠ (may need reinstall)"
...
echo "Run: crate ~/Music/TEST_DIR --dry-run -vv"
```
