...
0. **Not Knowing When to Kill a Feature**:
   - Problem: Sunk cost fallacy keeps buggy features alive
   - Rule: **3+ debugging sessions without resolution → cut the feature**
   - Example: TUI with persistent layout bugs → ship web-only instead
   - A polished single interface beats buggy multiple interfaces
